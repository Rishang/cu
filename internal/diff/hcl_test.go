...
func TestFormatHCL(t *testing.T) {
	cases := []struct {
		...
		{"int", int64(42), "42"},
		{"float", 3.14, "3.14"},
		{"integral float has no trailing zero", 2.0, "2"},
		...
		{"single pair", map[string]any{"k": "v"}, `{ k = "v" }`},
		{"pairs are sorted", map[string]any{"team": "infra", "env": "prod"},
			`{ env = "prod" team = "infra" }`},
		{"nested", map[string]any{"tags": map[string]any{"env": "prod"}},
			`{ tags = { env = "prod" } }`},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := formatHCL(tc.value); got != tc.want {
				t.Fatalf("formatHCL(%#v) = %q, want %q", tc.value, got, tc.want)
			}
		})
	}
}

// bool is not an int in Go, but this was a real trap in the Python version.
func TestFormatHCLBoolIsNotOne(t *testing.T) {
	if got := formatHCL(true); got != "true" {
		t.Errorf("formatHCL(true) = %q", got)
	}
	if got := formatHCL(int64(1)); got != "1" {
		t.Errorf("formatHCL(1) = %q", got)
	}
}
